-- Create a function to duplicate a project version atomically on the server
...
BEGIN
    ...
    -- 3. Clone Topics (maintaining hierarchy)
    -- We use a recursive approach or a loop with mapping to handle parent_id referencing
    -- For simplicity and performance, we'll use a loop that processes levels if we have many, 
    -- but here we'll use a mapping approach.
    
    -- First, insert all topics and store the mapping old_id -> new_id
    FOR t IN (
        SELECT * FROM topics 
        WHERE project_version_id = source_version_id
        ORDER BY parent_id NULLS FIRST -- Try to process parents before children
    ) LOOP
    ...
    -- 4. Clone Documents & Build mapping
    FOR d IN (
    ...
    -- We iterate through each doc, check if any links need rewriting.
    -- If they do, we create a NEW document_contents row.
    FOR d IN (
